...
thisText=`cat /vagrant/Guest_Scripts/setup_tor-A.txt |  cut -d" " -f2- | sed 's/ /\n/g' | awk '{a=$0;printf "%s ",a,$0}'`;
...
do sudo ip link set dev ${swpArray[0]}.100 master br0
do sudo ip link set dev ${swpArray[0]}.200 master br1
